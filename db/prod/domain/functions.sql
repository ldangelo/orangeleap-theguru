...
-- Create function to retrieve constituent custom fields
DROP FUNCTION IF EXISTS GETCONSTITUENTCUSTOMFIELD$$

CREATE FUNCTION `GETCONSTITUENTCUSTOMFIELD`(CONSTITUENTID INT, FIELDNAME VARCHAR(255))
	RETURNS VARCHAR(255)
BEGIN
	DECLARE FIELDVALUE VARCHAR(255);

	SELECT FIELD_VALUE INTO FIELDVALUE
		FROM CONSTITUENT_CUSTOM_FIELD
		JOIN CUSTOM_FIELD ON CUSTOM_FIELD.CUSTOM_FIELD_ID = CONSTITUENT_CUSTOM_FIELD.CUSTOM_FIELD_ID
