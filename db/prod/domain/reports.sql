...
SELECT 1, 'Codes - Account has a code type of [CODETYPE] and a code value of [CODEVALUE] or [CODEVALUE] or [CODEVALUE] or [CODEVALUE] or [CODEVALUE]',
'EXISTS (SELECT * FROM ENTITYCODES WHERE ENTITYCODES.ENTITYID = [VIEWNAME].ENTITYID AND CODETYPE = ''{0}'' AND CODEVALUE IN (''{1}'', ''{2}'', ''{3}'', ''{4}'', ''{5}''))';

INSERT INTO REPORTCUSTOMFILTERDEFINITION (REPORTCUSTOMFILTERDEFINITION_ID, DISPLAY_TEXT, SQL_TEXT)
SELECT 2, 'Segmentation - Account is in segmenation job number [JOBNUMBER] and group number [GROUPNUMBER]',
...
INSERT REPORTCUSTOMFILTERDEFINITION_REPORTDATASUBSOURCE VALUES (1, 7, 7, 1);
INSERT REPORTCUSTOMFILTERDEFINITION_REPORTDATASUBSOURCE VALUES (2, 7, 7, 2);
