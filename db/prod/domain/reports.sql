...
CREATE VIEW VPEOPLEGIFTDISTRO AS SELECT  AUTH_CODE, CHECK_NUMBER, COMMENTS, ORIGINAL_GIFT_ID,PAYMENT_TYPE, REFUND_GIFT_ID, REFUND_GIFT_TRANSACTION_DATE, TRANSACTION_DATE, GIFT.AMOUNT AS VALUE, ADDRESS_LINE_1, ADDRESS_LINE_2, ADDRESS_LINE_3, ADDRESS_TYPE, CITY, COUNTRY, POSTAL_CODE, STATE_PROVINCE, ANNIVERSARY, BIRTHDATE, EMAIL, FIRST_NAME, LAPSED_DONOR, LAST_NAME, MAJOR_DONOR, MARITAL_STATUS, MIDDLE_NAME, ORGANIZATION_NAME, PREFERRED_PHONE_TYPE, SPOUSE_FIRST_NAME, SPOUSE_NAME, SUFFIX, TITLE, VPEOPLEADDRESS.PERSON_ID, VPEOPLEADDRESS.ADDRESS_ID, DISTRO_LINE.AMOUNT, DISTRO_LINE.MOTIVATION_CODE, DISTRO_LINE.PROJECT_CODE FROM GIFT LEFT JOIN DISTRO_LINE ON GIFT.GIFT_ID = DISTRO_LINE.GIFT_ID LEFT JOIN VPEOPLEADDRESS ON GIFT.PERSON_ID = VPEOPLEADDRESS.PERSON_ID;

-- Create vgiftdistro
CREATE VIEW VGIFTDISTRO AS SELECT AUTH_CODE, CHECK_NUMBER, COMMENTS, ORIGINAL_GIFT_ID,PAYMENT_TYPE, REFUND_GIFT_ID, REFUND_GIFT_TRANSACTION_DATE, TRANSACTION_DATE, GIFT.AMOUNT AS VALUE, DISTRO_LINE.AMOUNT, DISTRO_LINE.MOTIVATION_CODE, DISTRO_LINE.PROJECT_CODE FROM GIFT LEFT JOIN DISTRO_LINE ON GIFT.GIFT_ID = DISTRO_LINE.GIFT_ID LEFT JOIN VPEOPLEADDRESS ON GIFT.PERSON_ID = VPEOPLEADDRESS.PERSON_ID;
...
'Gift Totals - Account has given at least $<input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[0]" value="{0}"/> in year <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[1]" value="{1}"/>';
...
'Codes - Account has a code type of <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[0]" /> with any code value';
...
'Codes - Account has a code type of <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[0]" /> with any of the following code values : <br/> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[1]" /> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[2]" /> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[3]" /> <BR> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[4]" /> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[5]"> <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[6]" />';

-- Account is in a segmentation
INSERT INTO REPORTCUSTOMFILTERDEFINITION (REPORTCUSTOMFILTERDEFINITION_ID, DISPLAY_TEXT, SQL_TEXT, DISPLAY_HTML)
SELECT 300, 'Segmentation - Account is in segmenation job number [JOBNUMBER] and group number [GROUPNUMBER]',
'EXISTS (SELECT * FROM S{0}G{1} SEGTABLE WHERE SEGTABLE.ENTITYID = [VIEWNAME].ENTITYID)',
'Segmentation - Account is in segmenation job number <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[0]" /> <br> and group number <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[1]" />';

-- Zip Radius
INSERT INTO REPORTCUSTOMFILTERDEFINITION (REPORTCUSTOMFILTERDEFINITION_ID, DISPLAY_TEXT, SQL_TEXT, DISPLAY_HTML)
SELECT 400, 'Zip Radius - Account is within [ZIPRADIUS] miles of zip code [ZIPCODE]',
...
'Zip Radius - Account is within <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[0]" /> miles of zip code <input objectname="reportFilters[INDEXREPLACEMENT].reportCustomFilter.reportCustomFilterCriteria[1]" />';
