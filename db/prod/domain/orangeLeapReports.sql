...
AS
	SELECT
		...
		(SELECT CODE_DESCRIPTION FROM CODE_TYPE JOIN CODE ON CODE.CODE_TYPE_ID = CODE_TYPE.ID
			WHERE CODE_TYPE.SITE_NAME = PERSON.SITE_NAME
			AND CODE_TYPE.NAME = 'projectCode'
			AND CODE.CODE_VALUE = DISTRO_LINE.PROJECT_CODE) AS DISTRO_LINE_PROJECT_CODE_DESCRIPTION,
		CASE WHEN DISTRO_LINE.MOTIVATION_CODE IS NULL THEN DISTRO_LINE.OTHER_MOTIVATION ELSE DISTRO_LINE.MOTIVATION_CODE END AS DISTRO_LINE_MOTIVATION_CODE,
		CASE WHEN DISTRO_LINE.MOTIVATION_CODE IS NULL THEN ''
		ELSE
		(SELECT CODE_DESCRIPTION FROM CODE_TYPE JOIN CODE ON CODE.CODE_TYPE_ID = CODE_TYPE.ID
			WHERE CODE_TYPE.SITE_NAME = PERSON.SITE_NAME
			AND CODE_TYPE.NAME = 'motivationCode'
			AND CODE.CODE_VALUE = DISTRO_LINE.MOTIVATION_CODE) 
		END AS DISTRO_LINE_MOTIVATION_CODE_DESCRIPTION
	FROM GIFT
	LEFT JOIN PAYMENT_SOURCE ON GIFT.PAYMENT_SOURCE_ID = PAYMENT_SOURCE.PAYMENT_SOURCE_ID
	LEFT JOIN ADDRESS GIFT_PAYMENT_ADDRESS ON GIFT.ADDRESS_ID = GIFT_PAYMENT_ADDRESS.ADDRESS_ID
	LEFT JOIN PHONE GIFT_PAYMENT_PHONE ON GIFT.PHONE_ID = GIFT_PAYMENT_PHONE.PHONE_ID
	...
	LEFT JOIN PERSON ON PERSON.PERSON_ID = GIFT.PERSON_ID
;
...
AS
	...
	LEFT JOIN PERSON ON PERSON.PERSON_ID = COMMITMENT.PERSON_ID
	WHERE COMMITMENT.COMMITMENT_TYPE = 'PLEDGE'
;
