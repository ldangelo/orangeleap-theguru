...
<html>
<head>
<meta name="pageHeading" content='<spring:message code="jsp.pageHeading.createReportWizard"/>'/>
<title><spring:message code="jsp.inputControlSource.title"/></title>
<script>
...
<table align="center" cellspacing="1" cellpadding="0" border="0">
	...
	<tr>
	  <td>&nbsp;</td>
	  <td colspan="2"><span class="fsection"><spring:message code="jsp.inputControlSource.header"/></span></td>
	</tr>
	<tr>
		<td colspan="3">&nbsp;</td>
	</tr>
	<spring:bind path="wrapper.inputControlSource">
		<tr>
			<td>&nbsp;</td>
			<td><input name="${status.expression}" type="radio" value="CONTENT_REPOSITORY" <c:if test='${status.value!="LOCAL" && !empty wrapper.inputControlList}'>checked="true"</c:if> <c:if test="${empty wrapper.inputControlList}">disabled</c:if>></td>
			<td><spring:message code="jsp.inputControlSource.fromRepository"/></td>
		</tr>
	</spring:bind>
	<spring:bind path="wrapper.inputControlPath">
		<tr>
			<td colspan="2">&nbsp;</td>
			<td>
				<select name="${status.expression}" size="1"<c:if test="${empty wrapper.inputControlList}">disabled</c:if> class="fnormal">
				<%--<c:if test='${status.value==null}'><option selected="true">Select Data Type</option></c:if>--%>
					<c:forEach items='${wrapper.inputControlList}' var='path'>
	...
	<spring:bind path="wrapper.inputControlSource">
		<tr>
			<td>&nbsp;</td>
			<td><input type="radio" name="${status.expression}" value="LOCAL" <c:if test='${status.value=="LOCAL" || empty wrapper.inputControlList}'>checked="true"</c:if>></td>
			<td><spring:message code="jsp.inputControlSource.locallyDefined"/></td>
		</tr>
	</spring:bind>
