...
WHERE VIEW_NAME IN ('VW_CONSTITUENTS_RECEIVECORRESPONDENCECONTACTINFORMATION');
...
VALUES
	('Constituents & Receive Correspondence Contact Information', 0, 'VW_V2_CONSTITUENTS_RECEIVECORRESPONDENCECONTACTINFORMATION', @REPORTDATASUBSOURCEGROUP_ID, 0, '/datasources/ReportWizardJdbcDS', '/datasources/ReportWizardJdbcDSSegmentationResults', 'Constituents and their addresses, email addresses and phone numbers that are marked to receive correspondence');
...
SELECT
	'VW_V2_CONSTITUENTS_RECEIVECORRESPONDENCECONTACTINFORMATION', 'Constituents & Receive Correspondence Contact Information', 'VW_V2_CONSTITUENTS', TRUE, 'VW_V2_CONSTITUENTS', '', TRUE, 100, 'ADDRESS.ADDRESS_ID IS NOT NULL OR EMAIL.EMAIL_ID IS NOT NULL OR PHONE.PHONE_ID IS NOT NULL';
...
SELECT
	@VIEW_ID, 'LEFT', 'ADDRESS', FALSE, 'ADDRESS', 'ADDRESS_', 'VW_V2_CONSTITUENTS.CONSTITUENT_CONSTITUENT_ID = ADDRESS.CONSTITUENT_ID AND IFNULL(ADDRESS.RECEIVE_CORRESPONDENCE, 0) = 1 AND ADDRESS.IS_DELETED = 0', TRUE, 1, '';
...
INSERT THEGURU_VIEW_JOIN
	(VIEW_ID, JOIN_TYPE, JOIN_TABLE, JOIN_TABLE_IS_VIEW, JOIN_TABLE_ALIAS, JOIN_TABLE_COLUMN_PREFIX, JOIN_CRITERIA, INCLUDE_ALL_FIELDS, SORT_ORDER, FIELD_GROUP_PREFIX)
SELECT
	@VIEW_ID, 'LEFT', 'EMAIL', FALSE, 'EMAIL', 'EMAIL_', 'VW_V2_CONSTITUENTS.CONSTITUENT_CONSTITUENT_ID = EMAIL.CONSTITUENT_ID AND IFNULL(EMAIL.RECEIVE_CORRESPONDENCE, 0) = 1 AND EMAIL.IS_DELETED = 0', TRUE, 3, '';
...
INSERT THEGURU_VIEW_JOIN
	(VIEW_ID, JOIN_TYPE, JOIN_TABLE, JOIN_TABLE_IS_VIEW, JOIN_TABLE_ALIAS, JOIN_TABLE_COLUMN_PREFIX, JOIN_CRITERIA, INCLUDE_ALL_FIELDS, SORT_ORDER, FIELD_GROUP_PREFIX)
SELECT
	@VIEW_ID, 'LEFT', 'PHONE', FALSE, 'PHONE', 'PHONE_', 'VW_V2_CONSTITUENTS.CONSTITUENT_CONSTITUENT_ID = PHONE.CONSTITUENT_ID AND (IFNULL(PHONE.RECEIVE_CORRESPONDENCE, 0) = 1 OR IFNULL(PHONE.RECEIVE_CORRESPONDENCE_TEXT, 0) = 1) AND PHONE.IS_DELETED = 0', TRUE, 5, '';
...
CALL GENERATE_VIEWS('VW_V2_CONSTITUENTS_RECEIVECORRESPONDENCECONTACTINFORMATION');
CALL GENERATE_FIELD_DEFINITIONS('VW_V2_CONSTITUENTS_RECEIVECORRESPONDENCECONTACTINFORMATION');
