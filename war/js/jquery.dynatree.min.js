// jquery.dynatree.js build 0.3.0_rc1_20081102
// Revision: 77, date: 2008-11-02 19:04:54
...
this.span.id=this.tree.options.idPrefix+this.data.key;this.span.className=(this.data.isFolder)?this.tree.options.classnames.folder:this.span.className=this.tree.options.classnames.document;this.div.appendChild(this.span);if(this.parent)
...
ac[i]._expand(false);}},onClick:function(event){if($(event.target).hasClass(this.tree.options.classnames.expander)){this.toggleExpand();}else if(this.data.isFolder&&this.tree.options.selectExpandsFolders&&(this.aChilds||this.data.isLazy)&&(this.parent!=null||this.tree.options.rootCollapsible)){this.toggleExpand();}else if(!this.data.isFolder){this.select();}else{}
this.focus();return false;},onKeypress:function(event){var code=(!event.charCode)?1000+event.keyCode:event.charCode;var handled=true;switch(code){case 43:if(!this.bExpanded)this.toggleExpand();break;case 45:if(this.bExpanded)this.toggleExpand();break;case 1032:case 32:this.select();break;case 1008:if(this.parent)
...
this.tree.options.onFocus.call(this.span,this);$(this.tree.tnFocused.span).addClass(this.tree.options.classnames.focused);}},_addChildNode:function(dtnode){if(this.aChilds==null)
...
this.render(true,true);return dtnode;},_addNode:function(data){var dtnode=new DynaTreeNode(this.tree,data);return this._addChildNode(dtnode);},append:function(obj){if(!obj)return;if(!obj.length)
...
$this.bind("click",fnClick);if(opts.keyboard){$this.bind("keypress keydown",fnKeyHandler);var div=this.tree.divTree;if(div.addEventListener){div.addEventListener("focus",fnFocusHandler,true);div.addEventListener("blur",fnFocusHandler,true);}else{div.onfocusin=div.onfocusout=fnFocusHandler;}
