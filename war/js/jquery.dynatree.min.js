// jquery.dynatree.js build 0.4.0_beta-1_20090202
// Revision: 166, date: 2009-02-02 08:43:12
// Copyright (c) 2008-09  Martin Wendt (http://dynatree.googlecode.com/)
// Licensed under the MIT License.


var _bDebug=false;_bDebug=true;function logMsg(msg){if(_bDebug&&window.console&&window.console.log){var dt=new Date();var tag=dt.getHours()+":"+dt.getMinutes()+":"+dt.getSeconds()+"."+dt.getMilliseconds();arguments[0]=tag+" - "+arguments[0];try{window.console.log.apply(window.console,arguments);}catch(e){}}}
...
var DynaTreeNode=Class.create();DynaTreeNode.prototype={initialize:function(tree,data){this.tree=tree;if(typeof data=="string")
data={title:data};if(data.key==undefined)
data.key="_"+tree._nodeCount++;this.data=$.extend({},$.ui.dynatree.nodedatadefaults,data);this.parent=null;this.div=null;this.span=null;this.childList=null;this.isRead=false;this.hasSubSel=false;if(tree.initMode=="cookie"){if(this.tree.initActiveKey==this.data.key)
this.tree.activeNode=this;if(this.tree.initFocusKey==this.data.key)
this.tree.focusNode=this;this.bExpanded=($.inArray(this.data.key,this.tree.initExpandedKeys)>=0);this.bSelected=($.inArray(this.data.key,this.tree.initSelectedKeys)>=0);}else{if(data.activate)
this.tree.activeNode=this;if(data.focus)
this.tree.focusNode=this;this.bExpanded=(data.expand==true);this.bSelected=(data.select==true);}
if(this.bExpanded)
this.tree.expandedNodes.push(this);if(this.bSelected)
this.tree.selectedNodes.push(this);},toString:function(){return"dtnode<"+this.data.key+">: '"+this.data.title+"'";},toDict:function(recursive){var dict=$.extend({},this.data);dict.activate=(this.tree.activeNode===this);dict.focus=(this.tree.focusNode===this);dict.expand=this.bExpanded;dict.select=this.bSelected;if(recursive&&this.childList){dict.children=[];for(var i=0;i<this.childList.length;i++)
dict.children.push(this.childList[i].toDict(true));}else{delete dict.children;}
...
this.span.innerHTML=this._getInnerHtml();this.div.style.display=(this.parent==null||this.parent.bExpanded?"":"none");this.span.className=(this.data.isFolder)?this.tree.options.classNames.folder:this.tree.options.classNames.document;if(this.bExpanded)
$(this.span).addClass(this.tree.options.classNames.expanded);if(this.data.isLazy&&!this.isRead)
$(this.span).addClass(this.tree.options.classNames.lazy);if(this.isLastSibling())
$(this.span).addClass(this.tree.options.classNames.lastsib);if(this.bSelected)
$(this.span).addClass(this.tree.options.classNames.selected);if(this.hasSubSel)
$(this.span).addClass(this.tree.options.classNames.partsel);if(this.tree.activeNode===this)
$(this.span).addClass(this.tree.options.classNames.active);if(this.data.addClass)
$(this.span).addClass(this.data.addClass);if(bDeep&&this.childList&&(bHidden||this.bExpanded)){for(var i=0;i<this.childList.length;i++){this.childList[i].render(bDeep,bHidden)}}},hasChildren:function(){return this.childList!=null;},isLastSibling:function(){var p=this.parent;if(!p)return true;return p.childList[p.childList.length-1]===this;},prevSibling:function(){if(!this.parent)return null;var ac=this.parent.childList;for(var i=1;i<ac.length;i++)
if(ac[i]===this)
...
this.childList.shift();}}else if(firstChild){data.isStatusNode=true;firstChild.data=data;firstChild.render(false,false);}else{data.isStatusNode=true;firstChild=this._addChildNode(new DynaTreeNode(this.tree,data));}},setLazyNodeStatus:function(lts){switch(lts){case DTNodeStatus_Ok:this._setStatusNode(null);this.isRead=true;this.render(false,false);if(this.tree.options.autoFocus){if(this===this.tree.tnRoot&&!this.tree.options.rootVisible&&this.childList){this.childList[0].focus();}else{this.focus();}}
...
parents[i]._expand(true);},focus:function(){this.makeVisible();try{$(this.span).find(">a").focus();}catch(e){}},isActive:function(){return(this.tree.activeNode===this);},activate:function(){var opts=this.tree.options;if(opts.onQueryActivate&&opts.onQueryActivate.call(this.span,true,this)==false)
return;if(this.data.isStatusNode)
...
$.cookie(opts.cookieId+"-active",this.data.key);$(this.span).addClass(opts.classNames.active);if(opts.onActivate)
...
$.cookie(opts.cookieId+"-active","");this.tree.activeNode=null;if(opts.onDeactivate)
...
if(activate){this.activate();}},_setSubSel:function(hasSubSel){if(hasSubSel){this.hasSubSel=true;$(this.span).addClass(this.tree.options.classNames.partsel);}else{this.hasSubSel=false;$(this.span).removeClass(this.tree.options.classNames.partsel);}},_select:function(sel,fireEvents,deep){var opts=this.tree.options;if(this.data.isStatusNode)
return;if(this.bSelected==sel){logMsg("dtnode._select(%o) IGNORED - %o",sel,this);return;}
...
this.tree.selectedNodes[0]._select(false,false,false);this.bSelected=sel;this.tree._changeNodeList("select",this,sel);if(sel){$(this.span).addClass(opts.classNames.selected);if(deep&&opts.selectMode==3){this.visit(function(dtnode){dtnode.parent._setSubSel(true);dtnode._select(true,false,false);});var p=this.parent;while(p){p._setSubSel(true);var allChildsSelected=true;for(var i=0;i<p.childList.length;i++){if(!p.childList[i].bSelected){allChildsSelected=false;break;}}
if(allChildsSelected)
p._select(true,false,false);p=p.parent;}}
if(fireEvents&&opts.onSelect)
opts.onSelect.call(this.span,true,this);}else{$(this.span).removeClass(opts.classNames.selected);if(deep&&opts.selectMode==3){this._setSubSel(false);this.visit(function(dtnode){dtnode._setSubSel(false);dtnode._select(false,false,false);});var p=this.parent;while(p){p._select(false,false,false);var isPartSel=false;for(var i=0;i<p.childList.length;i++){if(p.childList[i].bSelected||p.childList[i].hasSubSel){isPartSel=true;break;}}
p._setSubSel(isPartSel);p=p.parent;}}
if(fireEvents&&opts.onSelect)
opts.onSelect.call(this.span,false,this);}},isSelected:function(){return this.bSelected;},select:function(sel){return this._select(sel!=false,true,true);},toggleSelect:function(){return this.select(!this.bSelected);},_expand:function(bExpand){if(this.bExpanded==bExpand){logMsg("dtnode._expand(%o) IGNORED - %o",bExpand,this);return;}
var opts=this.tree.options;if(!bExpand&&this.getLevel()<opts.minExpandLevel){logMsg("dtnode._expand(%o) forced expand - %o",bExpand,this);return;}
if(opts.onQueryExpand&&opts.onQueryExpand.call(this.span,bExpand,this)==false)
return;this.bExpanded=bExpand;this.tree._changeNodeList("expand",this,bExpand);if(bExpand){$(this.span).addClass(opts.classNames.expanded);}else{$(this.span).removeClass(opts.classNames.expanded);}
if(this.bExpanded&&this.parent&&opts.autoCollapse){var parents=this._parentList(false,true);for(var i=0;i<parents.length;i++)
...
if(bExpand&&this.data.isLazy&&!this.isRead){try{this.setLazyNodeStatus(DTNodeStatus_Loading);if(true==opts.onLazyRead.call(this.span,this)){this.setLazyNodeStatus(DTNodeStatus_Ok);}}catch(e){this.setLazyNodeStatus(DTNodeStatus_Error);}
return;}
if(opts.fx){var duration=opts.fx.duration||200;$(">DIV",this.div).animate(opts.fx,duration);}else{$(">DIV",this.div).toggle();}},toggleExpand:function(){if(!this.childList&&!this.data.isLazy)
return;if(this.parent==null&&this.tree.options.minExpandLevel>0&&this.bExpanded)
return;this._expand(!this.bExpanded);},collapseSiblings:function(){if(this.parent==null)
...
$.cookie(opts.cookieId+"-focus",null);}else if(event.type=="focus"||event.type=="focusin"){if(this.tree.tnFocused&&this.tree.tnFocused!==this){logMsg("dtnode.onFocus: out of sync: curFocus: %o",this.tree.tnFocused);$(this.tree.tnFocused.span).removeClass(opts.classNames.focused);}
...
$.cookie(opts.cookieId+"-focus",this.data.key);}},_postInit:function(){if(opts.onPostInit)
...
this.childList=null;if(!recursive){this._expand(false);this.isRead=false;this.render(false,false);}}},_addChildNode:function(dtnode){if(this.childList==null){this.childList=new Array();}else{$(this.childList[this.childList.length-1].span).removeClass(this.tree.options.classNames.lastsib);}
this.childList.push(dtnode);dtnode.parent=this;if(((this.tree.options.minExpandLevel>0||!this.tree.options.rootVisible)&&this.parent==null)||(dtnode.data.expand)){this.bExpanded=true;}
...
var DynaTree=Class.create();DynaTree.version="$Version: 0.4.0_beta-1$";DynaTree.prototype={initialize:function(id,options){this.options=options;this.bEnableUpdate=true;this._nodeCount=0;this.initMode="data";this.activeNode=null;this.selectedNodes=new Array();this.expandedNodes=new Array();if(this.options.persist){this.initActiveKey=$.cookie(this.options.cookieId+"-active");if(cookie||this.initActiveKey!=null)
this.initMode="cookie";this.initFocusKey=$.cookie(this.options.cookieId+"-focus");var cookie=$.cookie(this.options.cookieId+"-expand");this.initExpandedKeys=cookie?cookie.split(","):[];cookie=$.cookie(this.options.cookieId+"-select");this.initSelectedKeys=cookie?cookie.split(","):[];}
logMsg("initMode: %o, active: %o, expanded: %o, selected: %o",this.initMode,this.initActiveKey,this.initExpandedKeys,this.initSelectedKeys);this.cache={tagEmpty:"<span class='"+options.classNames.empty+"'></span>",tagVline:"<span class='"+options.classNames.vline+"'></span>",tagExpander:"<span class='"+options.classNames.expander+"'></span>",tagConnector:"<span class='"+options.classNames.connector+"'></span>",tagNodeIcon:"<span class='"+options.classNames.nodeIcon+"'></span>",tagCheckbox:"<span class='"+options.classNames.checkbox+"'></span>",lastentry:undefined};this.divTree=document.getElementById(id);this.tnRoot=new DynaTreeNode(this,{title:this.options.title,key:this.options.idPrefix+"root"});this.tnRoot.data.isFolder=true;this.tnRoot.render(false,false);this.divRoot=this.tnRoot.div;this.divRoot.className=this.options.classNames.container;this.divTree.appendChild(this.divRoot);},toString:function(){return"DynaTree '"+this.options.title+"'";},toDict:function(){return this.tnRoot.toDict(true);},isInitializing:function(){return(this.initMode=="data"||this.initMode=="cookie"||this.initMode=="postInit");},_changeNodeList:function(mode,node,bAdd){if(!node)
...
if(this.options.persist){var keyList=$.map(nodeList,function(e,i){return e.data.key});$.cookie(cookieName,keyList.join(","));}else{}},redraw:function(){logMsg("dynatree.redraw()...");this.tnRoot.render(true,true);logMsg("dynatree.redraw() done.");},getRoot:function(){return this.tnRoot;},getNodeByKey:function(key){var el=document.getElementById(this.options.idPrefix+key);return(el&&el.dtnode)?el.dtnode:null;},getActiveNode:function(){return this.activeNode;},getSelectedNodes:function(){return this.selectedNodes;},activateKey:function(key){var dtnode=this.getNodeByKey(key);if(!dtnode){this.activeNode=null;return null;}
...
this.redraw();return!bEnable;},_createFromTag:function(parentTreeNode,$ulParent){var self=this;$ulParent.find(">li").each(function(){var $li=$(this);var $liSpan=$li.find(">span:first");var title;if($liSpan.length){title=$liSpan.html();}else{title=$.trim($li.html().match(/.*(<ul)?/)[0]);}
...
childNode=parentTreeNode._addNode(data);var $ul=$li.find(">ul:first");if($ul.length){self._createFromTag(childNode,$ul);}});},lastentry:undefined};$.widget("ui.dynatree",{init:function(){logMsg("ui.dynatree.init() was called, you should upgrade to ui.core.js v1.6 or higher.");return this._init();},_init:function(){logMsg("Dynatree._init(): version='%s'.",DynaTree.version);this.options.event+=".dynatree";var $this=this.element;var opts=this.options;if(!opts.imagePath){$("script").each(function(){if(this.src.search(/.*dynatree[^/]*\.js$/i)>=0){if(this.src.indexOf("/")>=0)
...
var id=$this.attr("id");this.tree=new DynaTree(id,opts);var root=this.tree.getRoot();if(opts.children){root.append(opts.children);}else if(opts.initAjax&&opts.initAjax.url){root.appendAjax(opts.initAjax);}else if(opts.initId){this.tree._createFromTag(root,$("#"+opts.initId));}else{var $ul=$this.find(">ul").hide();this.tree._createFromTag(root,$ul);$ul.remove();}
this.bind();this.tree.initMode="postInit";var nodeList=this.tree.expandedNodes;this.tree.expandedNodes=[];for(var i=0;i<nodeList.length;i++){var dtnode=nodeList[i];logMsg("Expand on init: %o",dtnode);dtnode._expand(true);}
nodeList=this.tree.selectedNodes;this.tree.selectedNodes=[];for(var i=0;i<nodeList.length;i++){var dtnode=nodeList[i];logMsg("Select on init: %o",dtnode);dtnode.bSelected=false;dtnode.select(true);}
if(this.tree.focusNode){logMsg("Focus on init: %o",dtnode);this.tree.focusNode.focus();}
if(this.tree.activeNode){var dtnode=this.tree.activeNode;this.tree.activeNode=null;logMsg("Activate on init: %o",dtnode);dtnode._userActivate();}
...
$this.bind("click.dynatree dblclick.dynatree keypress.dynatree keydown.dynatree",function(event){var dtnode=__getNodeFromElement(event.target);logMsg("bind("+event.type+"): dtnode:"+this+", charCode:"+event.charCode+", keyCode: "+event.keyCode+", which: "+event.which);if(!dtnode)
return false;switch(event.type){case"click":return(o.onClick&&o.onClick(dtnode,event)===false)?false:dtnode.onClick(event);case"dblclick":return(o.onDblClick&&o.onDblClick(dtnode,event)===false)?false:dtnode.onDblClick(event);case"keydown":return(o.onKeydown&&o.onKeydown(dtnode,event)===false)?false:dtnode.onKeydown(event);case"keypress":return(o.onKeypress&&o.onKeypress(dtnode,event)===false)?false:dtnode.onKeypress(event);};});function __focusHandler(event){event=arguments[0]=$.event.fix(event||window.event);var dtnode=__getNodeFromElement(event.target);return dtnode?dtnode.onFocus(event):false;}
var div=this.tree.divTree;if(div.addEventListener){div.addEventListener("focus",__focusHandler,true);div.addEventListener("blur",__focusHandler,true);}else{div.onfocusin=div.onfocusout=__focusHandler;}},unbind:function(){this.element.unbind(".dynatree");},enable:function(){this.bind();this._setData("disabled",false);},disable:function(){this.unbind();this._setData("disabled",true);},getTree:function(){return this.tree;},getRoot:function(){return this.tree.getRoot();},getActiveNode:function(){return this.tree.getActiveNode();},getSelectedNodes:function(){return this.tree.getSelectedNodes();},lastentry:undefined});$.ui.dynatree.getter="getTree getRoot getActiveNode getSelectedNodes";$.ui.dynatree.defaults={title:"Dynatree root",rootVisible:false,minExpandLevel:1,imagePath:null,children:null,initId:null,initAjax:null,autoFocus:true,keyboard:true,persist:false,autoCollapse:false,clickFolderMode:3,activeVisible:true,checkbox:false,selectMode:2,fx:null,onClick:null,onDblClick:null,onKeydown:null,onKeypress:null,onFocus:null,onBlur:null,onQueryActivate:null,onQuerySelect:null,onQueryExpand:null,onActivate:null,onDeactivate:null,onSelect:null,onExpand:null,onCollapse:null,onLazyRead:null,ajaxDefaults:{cache:false,dataType:"json"},strings:{loading:"Loading&#8230;",loadError:"Load error!"},idPrefix:"ui-dynatree-id-",cookieId:"ui-dynatree-cookie",classNames:{container:"ui-dynatree-container",folder:"ui-dynatree-folder",document:"ui-dynatree-document",empty:"ui-dynatree-empty",vline:"ui-dynatree-vline",expander:"ui-dynatree-expander",connector:"ui-dynatree-connector",checkbox:"ui-dynatree-checkbox",nodeIcon:"ui-dynatree-icon",nodeError:"ui-dynatree-statusnode-error",nodeWait:"ui-dynatree-statusnode-wait",hidden:"ui-dynatree-hidden",active:"ui-dynatree-active",selected:"ui-dynatree-selected",expanded:"ui-dynatree-expanded",lazy:"ui-dynatree-lazy",focused:"ui-dynatree-focused",partsel:"ui-dynatree-partsel",lastsib:"ui-dynatree-lastsib"},debugLevel:0,lastentry:undefined};$.ui.dynatree.nodedatadefaults={title:null,key:null,isFolder:false,isLazy:false,tooltip:null,icon:null,addClass:null,activate:false,focus:false,expand:false,select:false,children:null,lastentry:undefined};})(jQuery);