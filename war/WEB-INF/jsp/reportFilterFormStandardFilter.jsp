			...
			<SELECT objectname="reportFilters[INDEXREPLACEMENT].reportStandardFilter.fieldId"
				...
				<c:forEach var="fgroup" items="${fieldGroups}" varStatus="outer">
					<optgroup label="${fgroup.name}">
						<c:forEach var="f" items="${fgroup.fields}" varStatus="inner"><c:if test="${f != null }"><option label="${f.displayName}" value="${f.id}" fieldType="${f.fieldType}"<c:if test="${(currentFilter != null) && (currentFilter.reportStandardFilter.fieldId == f.id)}">selected="true"<c:if test="${f.fieldType == 'DATE'}"><c:set var="dateFieldSelected" scope="page" value="true"/></c:if></c:if>>${f.displayName}</option></c:if>
						</c:forEach>
					</optgroup>
				</c:forEach>
			</SELECT>
